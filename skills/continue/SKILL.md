...
2. 直接依据已注入的进度和角色状态快照确认最近章节摘要、角色当前位置、身体、心理、目标、持有物和关系等当前状态；这些快照无需重复读取，只有注入被截断需要全文时才按路径读取
3. 如 `setting/chapter-groups/` 下存在当前章节组细纲，优先读取对应细纲，确认本章在组内的作用、冲突、信息揭示和钩子
4. 先用章节组细纲和进度的短期衔接提示判断本章叙事定位，再决定衔接对象：常规线性推进时读取最近 1-2 章实际正文核对连贯；开启新卷、新副本/新单元、单元剧或番外等相对独立的叙事单元时，衔接该单元自身的起点、相关设定和必要跨单元线索，而不是机械读上一章

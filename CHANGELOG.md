...
- 新增 Agent 字数统计工具 `count_words`：写作 Agent 与自动化（章节批量审阅）Agent 现在可以按与界面完全一致的口径（一个非空白字符记 1 字，中文按字数统计）获取字数，解决 review 输出中模型自行估算字数不准的问题。不传参数返回全书统计（每章字数、章节总字数、大纲/细纲文档字数）；传入 `paths` 统计指定文件（绝对或相对路径，缺失文件按条目报错）；可选 `start_line` / `end_line`（1-based、含端点、与 `read_file` / `edit_file` 行号一致）把统计限定到指定行区间，用于统计章节内某一段，`end_line` 省略时统计到末行并在结果中回显实际生效范围。工具为纯读操作，受 Agent Tools 的 `file_read` 能力开关约束，可与其他读工具并行执行。Reviewer SubAgent 同样可用。
- Added an Agent word-count tool `count_words`: the Writing Agent and Automation agents (chapter batch review) can now report word counts using the exact same rule as the UI (one non-whitespace character = one word; CJK counts per character), fixing inaccurate model-estimated counts in review output. Called without arguments it returns whole-book statistics (per-chapter words, chapter total, outline/detailed-outline document words); with `paths` it counts specific files (absolute or relative; missing files report per-entry errors); optional `start_line` / `end_line` (1-based, inclusive, same line numbers as `read_file` / `edit_file`) restrict counting to a line range for measuring a section inside a chapter — omitting `end_line` counts through the last line, and the effective range is echoed back in the result. The tool is read-only, gated by the `file_read` capability in Agent Tools, and runs in parallel with other read tools. The Reviewer SubAgent can use it too.

- 思考过程默认收起：新增用户级配置「默认展开思考过程」（`chat_thinking_expanded_default`，设置 → 外观，默认关闭）。默认关闭时，聊天区「思考过程」折叠块在流式输出期间与结束后都保持收起，需要时手动展开；开启后保持原有行为（流式期间自动展开、结束后自动收起）。任何状态下手动展开/收起都不被覆盖。用户可在用户层与工作区层分别覆盖（继承规则与其他外观设置一致）。写作与游戏模式及 SubAgent 输出共用同一渲染路径，行为一致。
- Thinking blocks now default to collapsed: a new user-level setting "Expand Thinking by Default" (`chat_thinking_expanded_default`, Settings → Appearance, off by default). When off, the "Thinking" collapsible in chat stays collapsed both during streaming and after completion, expanding only on demand; when on, the previous behavior is kept (auto-expand while streaming, auto-collapse when done). Manual toggling is never overridden in either mode. The setting supports user and workspace layers with the same inheritance rules as other appearance options. Writing Mode, Interactive Mode, and SubAgent output share the same render path and behave identically.

- `edit_file` 新增优先按行修改协议：`read_file` 会为每条真实源文件行返回稳定的 1-based 行号和全文 `revision`；模型可把该 revision 作为 `file_revision`，再用 `start_line` / `end_line`（包含结束行）与 `new_string` 批量替换完整行，不再需要回传大段 `old_string`。同一次调用的所有范围都基于同一原始快照解析并原子提交，从后向前应用以避免行号漂移；范围越界、互相重叠或 revision 过期时整批拒绝。行替换会保持 LF/CRLF 与末行换行语义，空 `new_string` 可删除所选行。原有 `old_string` / `replace_all` 精确替换继续兼容，并作为行号不适合定位时的备选。`read_file` 同时修复了文件以换行结尾时显示不存在的额外空行号的问题。
- `edit_file` now prefers revision-guarded line edits. `read_file` returns a stable 1-based number for every real source line plus the full-file `revision`; the model can pass that revision as `file_revision` and use inclusive `start_line` / `end_line` with `new_string` to replace complete lines without echoing large `old_string` payloads. Every range in one call is resolved against the same immutable snapshot and committed atomically, applying from the end so earlier replacements cannot shift later targets; out-of-range, overlapping, or stale-revision batches are rejected in full. Line edits preserve LF/CRLF and final-line termination semantics, and an empty `new_string` deletes the selected lines. Existing `old_string` / `replace_all` exact edits remain compatible as a fallback when line targeting is unsuitable. `read_file` also no longer displays a non-existent extra blank line number when a file ends with a newline.
- 大纲与细纲结构模板改为书内可编辑文件：每本作品初始化 `setting/outline-format.md` 与 `setting/chapter-group-format.md`，分别定义该书的大纲与细纲结构。它们与「大纲 / 规则 / 进度 / 灵感 / 状态」一起显示在书籍设定快捷区，作者可直接打开编辑，创作 Agent 也可通过文件工具修改；格式天然按书隔离，不再依赖设置页或 `.nova/config.toml`。两个文件缺失或留空时回落系统内置默认，注入前按 UTF-8 安全方式限制为 256 KiB；实际写作 system prompt、Agents 页流程预览与上下文分析统一使用同一份书内文件，避免展示与模型输入不一致。
- Outline and detailed-outline structure templates are now editable book files. Each book initializes `setting/outline-format.md` and `setting/chapter-group-format.md`, which define that book's outline and detailed-outline structures. They appear beside Outline / Rules / Progress / Ideas / State in Book Settings, can be edited directly by the author or through Agent file tools, and are naturally isolated per book instead of living in Settings or `.nova/config.toml`. Missing or blank files fall back to the built-in defaults; injected content is UTF-8-safely capped at 256 KiB. The runtime writing system prompt, Agents-page flow preview, and context analysis all resolve the same book files so displayed and model-visible inputs stay aligned.

- 新增写作模型自动分配（`writing_compute_tier`），并直接合并到输入框右下角原有模型选择器：同一个菜单先列出「指定模型」下的全部已配置模型，再列出 `Auto-平衡`、`Auto-质量优先`、`Auto-速度优先`。选择具体模型会写入内部 `manual` 模式，使主 Agent 与全部写作 SubAgent 强制使用该模型；选择 Auto 则按 SubAgent 的 `compute_role`（`prose`/`reasoning`/`mechanical`，展示名为正文/推理/检查）自动分配：`quality` 全部阶段继承主模型，`balanced`（默认）保留正文主模型并把推理/检查下放到 fast profile，`speed` 全部使用 fast profile 且仅正文保留思考。逐 SubAgent 的高级模型覆盖只在 Auto 模式生效。输入动作菜单不再提供重复的算力档位入口；Agents 页仍保留档位、快速模型和各阶段生效模型汇总作为高级配置。该能力只作用于写作模式，游戏模式不受影响；内置六个写作 SubAgent 已带默认 `compute_role`。中英文完整本地化。
- Added automatic model allocation for Writing (`writing_compute_tier`) directly into the existing model selector at the bottom-right of the composer. One menu lists every configured model under “Specified Model”, followed by `Auto-Balanced`, `Auto-Quality`, and `Auto-Speed`. Choosing a concrete model stores an internal `manual` mode that forces the main Agent and every Writing SubAgent to use that model. Choosing Auto allocates by each SubAgent’s `compute_role` (`prose`/`reasoning`/`mechanical`, shown as Prose/Reasoning/Check): `quality` inherits the main model for every stage, `balanced` (default) keeps prose on the main model while offloading reasoning/check stages to the fast profile, and `speed` uses the fast profile everywhere while keeping thinking only for prose. Advanced per-SubAgent model overrides apply only in Auto modes. The Input Actions menu no longer contains a duplicate compute-tier entry; the Agents page keeps tier, fast-model, and effective-stage summaries as advanced configuration. This applies only to Writing Mode; Interactive Mode is unaffected. The six built-in Writing SubAgents ship with default `compute_role`s. Fully localized in Chinese and English.
- `config.toml` 模板新增预置 `flash` 模型 profile（`id = "flash"`，默认 `deepseek-v4-flash`，`openai_api_key`/`openai_base_url` 留空则继承 default profile）：`balanced` / `speed` 档位开箱即可将推理与检查阶段下放到该快速模型。若删除或未配置该 profile，档位会自动回退到 `default`（pro），功能安全降级、不报错。
- The `config.toml` template now ships a preset `flash` model profile (`id = "flash"`, default `deepseek-v4-flash`, inheriting `openai_api_key`/`openai_base_url` from the default profile when left blank), so `balanced` / `speed` tiers can offload reasoning and mechanical stages to a fast model out of the box. If the profile is removed or unset, the tier safely falls back to `default` (pro) without errors.
- Auto 档位的「快速模型」可自选（`writing_compute_fast_profile_id`，默认 `flash`）：Agents 页（写作 → 用户层）可把 `balanced` / `speed` 下放阶段指向任意已配置 profile，并即时更新「各阶段生效模型」汇总与每个写作 SubAgent 编辑器的继承值；所选 profile 不存在时回退到 `default`（pro）安全降级。
- The fast model used by Auto tiers is selectable (`writing_compute_fast_profile_id`, default `flash`). On the Agents page (Writing → user layer), `balanced` / `speed` offloaded stages can target any configured profile, immediately updating the effective-stage summary and inherited values in each Writing SubAgent editor. A missing selected profile safely falls back to `default` (pro).

- 写作模式新增用户级「快捷创作」配置（设置 → 写作模式 → 快捷创作）：可修改按钮名称与提示词、新增或删除按钮、调整顺序、恢复内置默认，并支持在提示词中使用 `{target}` 动态引用当前章节、当前文件或当前作品。配置持久化到用户 `config.toml` 的 `writing_quick_actions`，最多保留 24 项，单条提示词上限 256K 字符；内置按钮与提示词完整支持中英文并随界面语言切换。
- Writing Mode now has user-level Quick Writing settings under Settings → Writing Mode → Quick Writing. Users can edit button labels and prompts, add or remove actions, reorder them, restore built-in defaults, and use `{target}` in prompts to reference the current chapter, file, or work dynamically. Customizations persist as `writing_quick_actions` in the user `config.toml`, with up to 24 actions and a 256K-character prompt limit per action; built-in labels and prompts are fully localized and follow the interface language.
...
- 修复模型"过度重读"问题的结构性根因：此前跨轮上下文装配只要发现文件 revision 变了——**包括模型自己刚做的 edit_file/write_file**——就把保留的 read_file 正文折叠成一张"请重读"的 receipt，于是模型在编辑循环中每一轮都不得不重新 read_file 才能拿到行号。现在装配时文件未变化仍逐字节保留原正文（缓存前缀稳定）；文件已变化则**原地刷新**为当前内容：按原 offset/limit 重选窗口、更新元数据 revision 并标记 `"refreshed": true`，模型下一轮开始时上下文里的带行号正文永远是当前状态，配合编辑回执中的行号区间可以全程免重读。刷新仅在文件确实变化时发生（此时模型本来就必须重读，刷新省掉了整次工具调用与一轮推理）；窗口无法一致重读（文件被删、缩短越界、并发再变）时保守回退 receipt。预算语义同步扩展（`retained_prose_max_bytes`：未变化保留原文 / 已变化刷新为当前窗口，超预算仍折叠），旧会话已存的 receipt 不受影响。read_file 工具描述新增 refreshed 契约说明：上下文中的带行号正文（尤其 refreshed 的）就是权威当前快照，禁止为了"确认一下"而重读。
- Fixed the structural root cause of the model's excessive re-reading: cross-turn context assembly used to collapse a retained read_file body into a "please re-read" receipt whenever the file's revision moved — **including moves caused by the model's own edit_file/write_file** — so every turn of an editing loop began with a forced re-read just to recover line numbers. Assembly now keeps unchanged bodies verbatim (byte-identical cache prefix) and **refreshes** changed bodies in place: the same offset/limit window is re-selected from the current content, the metadata revision moves forward, and `"refreshed": true` is set, so the numbered body in context is always turn-current and the model can keep editing by line without re-reading, using the line ranges already reported by edit receipts. Refreshing only happens when the file actually changed (when the model would otherwise have to re-read anyway; the refresh saves the whole tool call and a reasoning round); an inconsistent window (file deleted, shrunk past the offset, or changed again mid-assembly) conservatively falls back to the receipt. The `retained_prose_max_bytes` budget now covers verbatim-unchanged and refreshed-current bodies alike (over-budget still collapses), and receipts persisted in older sessions are untouched. The read_file tool description documents the refreshed contract: any numbered body in context — especially a refreshed one — is the authoritative current snapshot, and re-reading "just to make sure" is forbidden.
- 全面收敛各处"命令模型重读"的提示文案，改为条件化指引：跨轮折叠 receipt 的提示从"Re-read this path/window"改为"仅在下一步确实需要原文时再读"；edit_file 调用参数投影的提示不再说"Re-read the current source of truth"；edit_file 工具描述的恢复条款从"行号越界、匹配失败或 revision 变化就读文件"改为"仅行号越界或 old_string 匹配失败且上下文正文仍当前时，直接从该正文修正选择器并重试，只有 revision 冲突或上下文缺少当前正文才重读"；审阅反馈提示从无条件"Re-read the latest file"改为优先使用上下文中已被装配保持为当前的带行号正文；写作系统提示同步加入"上下文已有带行号 read_file 正文时禁止为确认状态而重复读取"。修订冲突错误文案保持不变。
- Prompt copy across the stack no longer commands reflexive re-reading and is now conditional: the cross-turn collapse receipt note changes from "Re-read this path/window" to "read again only when its exact wording is needed for the next step"; the edit_file call-argument projection no longer says "Re-read the current source of truth"; the edit_file recovery clause changes from "read the file again on invalid range, failed match, or changed revision" to "fix the selector directly from the current numbered body in context and retry when only a range or match failed; re-read only on a revision conflict or a genuinely missing body"; the review-feedback prompt drops its unconditional "Re-read the latest file" in favor of the assembly-kept current numbered body; and the writing system prompt now forbids re-reading to "confirm the latest state" when a numbered body is already in context. Revision-conflict error wording is unchanged.
- revision 读写编辑链路全面提速，长会话与长历史工作区的重复读、重复哈希与事件风暴显著减少。`read_file` 的全文 revision 锚点改为进程级 stat 缓存（size+mtime 校验）：跨轮上下文装配与重复读取不再对未变化的文件做全量读+哈希，`edit_file` / `write_file` 成功后直接回填新 revision，任何外部修改都会因 stat 变化而失效重算（锚点仅作新鲜度提示，所有写入前的权威校验仍基于锁内新读字节，安全语义不变）。`edit_file` 仅含精确匹配编辑、或 `write_file` 未传 `file_revision` 时不再预读全文件：服务端在变更锁内直接以当前状态为基线（精确编辑以 `old_string` 自锚），省去每次调用一次全文件读+哈希；按行编辑仍强制要求 `file_revision`。`edit_file` / `write_file` 结果新增每个编辑的前后行号区间（1-based、含端点、最多 20 条以保持 receipt 有界），模型可直接据此平移后续行号而无需重读文件，工具描述（中英文）同步更新。后端 `workspacechange` 事件投影改为增量维护：应用一条事件不再全表扫描所有变更并克隆整组 ChangeSet（长历史工作区的账本重放从平方级降为线性），CanUndo/CanRedo 改为在读取边界按需校验当前文件头；blob 存储缓存目录句柄并跳过进程内已校验 blob 的重复读+哈希+目录 fsync（每次编辑的 before-blob 通常就是上一次的 after-blob）；多个已撤销组被一次新写失效时合并为一条 ledger 事件（原来 N 次串行 fsync → 1 次）。`SaveFile`（编辑器保存）仍强制显式 base revision，行为不变。注意两点不兼容：空 `base_revision` 的 `ApplyEdits`/`ReplaceFile` 从"报错拒绝"变为"以当前状态为基线"（行号编辑除外，仍报错）；批量 redo 失效事件带新字段 `history_group_ids`，旧版本二进制会忽略该字段（表现为降级运行时 redo 能力判断回退，写入安全不受影响），回滚到旧版本前请知悉。
- The revision read/write/edit pipeline is significantly faster, with far fewer repeated reads, repeated hashes, and event storms in long sessions and long-history workspaces. The `read_file` full-file revision anchor now uses a process-wide stat-validated cache (size+mtime): cross-turn context assembly and repeated reads no longer re-read and re-hash unchanged files, `edit_file` / `write_file` seed the new revision directly after success, and any external change invalidates the anchor through stat (the anchor is advisory only; every authoritative pre-write check still hashes freshly read bytes under the mutation lock, so safety semantics are unchanged). `edit_file` calls with only exact-match edits, and `write_file` calls without `file_revision`, no longer pre-read the whole file: the service anchors against the current state under its change lock (exact edits are anchored by `old_string` itself), saving a full read+hash per call; line-based edits still require `file_revision`. Successful `edit_file` / `write_file` results now include each edit's before/after line ranges (1-based, inclusive, capped at 20 entries to keep the receipt bounded), letting the model shift subsequent line numbers without re-reading; the tool descriptions (Chinese and English) are updated accordingly. Backend `workspacechange` event projection is now incremental: applying one event no longer rescans every change in the workspace and clones the whole group's ChangeSets (ledger replay for long-history workspaces drops from quadratic to linear), and CanUndo/CanRedo are validated lazily at read boundaries against the current file head; blob storage caches its directory handle and skips the repeated read+hash+directory fsync for blobs already verified in this process (every edit's before-blob is usually the previous after-blob); invalidating redo for multiple undone groups is now one ledger event instead of N serialized fsyncs. `SaveFile` (editor saves) still requires an explicit base revision, unchanged. Two breaking notes: an empty `base_revision` on `ApplyEdits`/`ReplaceFile` now means "anchor on the current state" instead of an error (line-based edits still error); and the batched redo-invalidation event carries a new `history_group_ids` field that older binaries ignore (degraded to a stale redo-capability hint, never a write-safety issue) — be aware before rolling back to an older build.

- `edit_file` / `write_file` 成功结果开始向模型返回修改后的文件 `revision`（仅隐藏修改前的 `base_revision`）：模型继续修改同一文件时可直接把它作为 `file_revision` 传入、免去重复 `read_file`，行号按自己的修改自行推算。工具描述（中英文）与写作系统提示同步改写：只有无法可靠推算当前行号（文件在本轮修改之外发生变化、或返回 revision 冲突）时才需要重新读取；revision 冲突的公开错误信息引导「使用最新一次 read_file 结果里的 revision，不要复用之前尝试或旧上下文快照里的值」。后端 `edit_file` 失败日志新增 `error_code`、`expected_revision`（模型提交值）与 `actual_revision`（磁盘当前值），仅记录于日志、不会暴露给模型，便于定位模型反复复用旧 revision 导致的冲突。数据模型不变。
- Successful `edit_file` / `write_file` results now return the file's post-edit `revision` to the model (only the pre-edit `base_revision` stays hidden), so the model can pass it as `file_revision` and keep editing the same file without re-reading, adjusting line numbers from its own edits. The tool description (Chinese and English) and the writing system prompt are rewritten accordingly: re-reading is needed only when current line numbers cannot be computed reliably (the file changed outside the model's own edits, or a revision conflict was reported); the public revision-conflict error now instructs the model to use the revision from the most recent `read_file` result and not to reuse a value from an earlier attempt or older context snapshot. Backend `edit_file` failure logs now include `error_code`, `expected_revision` (what the model sent) and `actual_revision` (current on disk) — log-only, never exposed to the model — to diagnose the repeated stale-revision retry pattern. The data model is unchanged.

- `edit_file` 强化按行修改优先规则：工具描述重构为「上下文中已有带编号的 read_file 正文及其 revision 时直接按行编辑、无需重读，缺失时才先 read_file」，并明确必须用 `start_line`/`end_line`（写作系统提示与审阅反馈提示同步收紧）；`old_string` 的适用场景被明确收窄为两类——同一行内部的局部修改、以及配合 `replace_all` 替换所有出现，且必须从最新 read_file 结果逐字复制、禁止凭记忆重建；`start_line`/`old_string` 的 JSON Schema 字段描述同步更新。后端为每次 `edit_file` 调用新增结构化日志（按行/精确修改数量、是否携带模型 revision、成败与 change_set_id），便于量化观察模型的选择器偏好；revision 冲突的公开错误信息改为同时提示「读取后文件发生变化（可能是你自己之前的修改）」，引导重新读取而非整文件覆盖。工具行为与数据模型不变，不影响已有会话与变更记录。
- `edit_file` now enforces the line-first rule more strongly: the tool description is restructured so the model edits by line directly without re-reading whenever the numbered `read_file` body and its revision are already in context, and reads first only when they are missing, requiring `start_line`/`end_line` (the writing system prompt and review-feedback prompt are tightened accordingly); `old_string` is narrowed to two explicit cases — a partial-line change inside one line, or `replace_all` to replace every occurrence — and must be copied verbatim from the latest `read_file` result, never reconstructed from memory; the `start_line`/`old_string` JSON-schema field descriptions are updated to match. The backend now logs every `edit_file` call structurally (line vs exact edit counts, whether the model supplied a revision, outcome and `change_set_id`) to quantify selector preference, and the public revision-conflict error now explains that the file changed after the read (possibly by the model's own earlier edit) and steers toward re-reading instead of a full-file rewrite. Tool behavior and the data model are unchanged; existing sessions and change records are unaffected.

- 内置大纲模板改为卷级，聚焦故事概览而非逐章清单：大纲现在先写故事概览（一句话简介、核心剧情、核心设定概述），再做分卷规划（每卷内容、关键节点、结束状态），只规划到卷级；逐章安排交给细纲（`setting/chapter-groups/`）。主要角色随一句话简介与核心剧情自然带出，大纲不再单列人物表，完整人物与世界设定统一放资料库，避免与资料库重复。此前内置模板是从卷一路铺到「每章一句话摘要」的清单，既缺故事概览又与细纲职责重叠，导致大纲显薄、像是靠正文反推。同时在写作系统提示中强化「灵感 → 大纲（故事概览+分卷）→ 细纲（逐章）→ 正文」的顺序，并加入软引导：大纲仍为空壳/占位时，进入正式章节创作前先建议作者补全大纲（建议而非硬性阻断）。仅影响新生成与模型认知，不迁移或改写已有大纲文件；游戏模式不使用大纲、不受影响。
- The built-in outline template is now volume-level and focuses on a story overview instead of a per-chapter list. The outline first captures a story overview (logline, core plot, core-setting summary), then a per-volume plan (each volume's content, key beats, and end state), stopping at the volume tier; per-chapter planning is handled by the detailed outline (`setting/chapter-groups/`). Main characters surface within the logline and core plot rather than a standalone roster, and full character/world profiles live in the Lore library to avoid duplication. Previously the built-in template ran all the way down to a "one-sentence summary per chapter", which lacked a story overview and overlapped with the detailed outline's responsibility, making the outline look thin and reverse-derived from prose. The writing system prompt also reinforces the "spark → outline (overview + volumes) → detailed outline (per chapter) → prose" ordering and adds soft guidance: when the outline is still empty or placeholder, suggest completing it before starting real chapters (guidance, not a hard block). This affects only new generation and model behavior; existing outline files are not migrated or rewritten. Interactive mode does not use outlines and is unaffected.

- 移除设置页与配置文件中的 `outline_format` / `chapter_group_format`：这两个模板现在只由书内 `setting/*.md` 文件承载。Beta 阶段不迁移旧用户级或工作区级配置值；已有配置会被忽略，需要时请把原内容复制到对应书籍文件。对已有书籍，打开工作区时会仅在文件缺失时创建默认模板，不覆盖作者已有内容。
- Removed `outline_format` / `chapter_group_format` from Settings and configuration files; these templates now live only in the book's `setting/*.md` files. During beta, legacy user/workspace config values are not migrated and are ignored; copy any needed content into the corresponding book files. Existing books receive default template files only when they are missing, and author content is never overwritten.

- 移除动作编排与亲密编排能力（`action-choreography` / `intimacy-choreography` 两个 Skill 及 `choreographer` / `intimacy-choreographer` 两个预置 SubAgent），因其触发门槛极高、对绝大多数续写没有实际收益，却增加了预设流程与配置的复杂度。复杂战斗、追逐、多人协作、灾害或复杂亲密等多主体、多节拍场景，现在由主 Agent（`novel-heavy` 下由 `writer` 依据 Context Plan 的「场景推演」要点）在正文里直接逐拍处理，仍要求空间连续、状态连续与反应因果，并对亲密场景保持"尺度随用户输入、主体成年自愿、迟疑即回应"的约束。通用的 Skill delegate 机制保留不变。默认配置不再内置任何 SubAgent；若用户此前在 `config.toml` 覆盖过这两个 SubAgent，相关条目将不再对应内置默认，可自行删除。
- Removed the action- and intimacy-choreography capability (the `action-choreography` / `intimacy-choreography` Skills and the preseeded `choreographer` / `intimacy-choreographer` SubAgents) because their trigger bar was very high and they added preset-flow and configuration complexity without real benefit for the vast majority of continuations. Complex multi-subject, multi-beat scenes (combat, chases, group coordination, disasters, or complex intimacy) are now handled inline by the main Agent — under `novel-heavy` by the `writer`, guided by the Context Plan's "scene reasoning" field — still preserving spatial continuity, state continuity, and reaction causality, and for intimacy keeping "explicitness follows the user's input, subjects are adult and consenting, and hesitation is addressed first". The general Skill delegate mechanism is unchanged. Default settings no longer ship any built-in SubAgent; if you previously overrode these two SubAgents in `config.toml`, those entries no longer map to a built-in default and can be removed.
- 移除 `orchestrate-projects` Skill：它是一套面向通用软件研发的项目协调方法论（GOALS.md 路线图、里程碑/代码评审、浏览器与桌面本地检查等），与小说创作场景不匹配，且它标记为写作模式可用，会持续占用模型的 Skill 选择空间却几乎不会被正确触发。写作的长期结构与进度协调由现有的 `outline.md` + `progress.md` + `chapter-groups/` 承担，用户输入的意图路由由写作意图分类链承担，均不依赖该 Skill。纯 Skill 删除，无代码耦合。
- Removed the `orchestrate-projects` Skill: it was a general software-project coordination methodology (GOALS.md roadmap, milestones/code review, local browser and desktop checks) that does not fit novel writing, and because it was marked available to Writing Mode it kept occupying the model's Skill-selection space while almost never triggering correctly. Long-term structure and progress coordination for writing is handled by the existing `outline.md` + `progress.md` + `chapter-groups/`, and user-intent routing by the writing intent-classification chain; neither depends on this Skill. Pure Skill removal with no code coupling.
- 清理面向用户与模型的第三方 Agent 名称，改为通用表述：规划模式提示中「像 Codex Plan Mode 一样」改为「在规划模式下」，前端变更摘要卡注释中的 "Codex-style" 改为通用说明。用于导入第三方工具 Skill 的兼容目录列表（含 `.codex/skills`、`.claude/skills`、`.cursor/skills` 等）保持不变，不影响从这些工具导入 Skill。
- Replaced user- and model-facing third-party agent names with generic wording: the plan-mode prompt's "like Codex Plan Mode" becomes "in plan mode", and the frontend change-summary card comment's "Codex-style" becomes a generic description. The interop directory list used to import Skills from third-party tools (including `.codex/skills`, `.claude/skills`, `.cursor/skills`, and others) is unchanged, so importing Skills from those tools still works.
- 写作模式跨轮保留 `read_file` 正文改为按文件 revision 判定，并停止让写作 Agent 重复读取已注入的作品状态。`read_file` 结果的元数据首行新增全文 revision 锚点（复用 `workspacechange.Revision`，超过 4MiB 的文件不带锚点）；跨轮上下文装配时，若文件自读取以来未变化，直接保留当轮完整正文（前缀缓存稳定、无需重复读取），仅在文件已变化、无锚点或超出保留预算时才回退为原有的 path/offset receipt 以触发定点重读。保留预算由新增的按 Agent 类型配置项 `retained_prose_max_bytes` 控制（写作模式默认 256KiB，其它 Agent 为 0 即维持始终 receipt 的旧行为），并按「最新优先」在预算内保留最近的未变化正文。同时对齐写作与游戏模式的注入约定：写作系统提示、`continue`/`group-plan`/`rewrite` Skill 与「写下一章」「下一组细纲」快捷提示不再命令重复 `read_file` 已注入的 `outline.md`/`progress.md`/`character-states.md`/资料库快照，只在需要被截断的全文或某章实际正文时才显式读取，向 progress/character-states/资料库写回的规则不变。旧会话中已存的 receipt 保持不变，向后兼容。
- Cross-turn retention of `read_file` bodies in Writing Mode is now revision-aware, and the writing Agent no longer re-reads already-injected workspace state. The `read_file` result metadata line gains a full-file revision anchor (reusing `workspacechange.Revision`; files above 4MiB carry no anchor). At context assembly, an unchanged file keeps its full source-turn body verbatim (cache-stable, no re-read), and only a changed file, a missing anchor, or an over-budget body falls back to the existing path/offset receipt that forces a targeted re-read. The retention budget is governed by a new per-agent-kind setting `retained_prose_max_bytes` (default 256KiB for Writing, 0 for other agents to preserve the always-receipt behavior), keeping the most recent unchanged bodies newest-first within budget. The injection contract is also aligned between Writing and Interactive modes: the writing system prompt, the `continue`/`group-plan`/`rewrite` Skills, and the "write next chapter"/"next chapter-group" quick prompts no longer instruct re-reading the already-injected `outline.md`/`progress.md`/`character-states.md`/Lore snapshots, reading explicitly only for truncated full text or a specific chapter's actual prose, while write-back rules for progress/character-states/Lore are unchanged. Receipts already persisted in older sessions stay unchanged for backward compatibility.
- 续写前"读哪些正文"改为由叙事结构驱动，而不是固定读上两章：写作系统提示、`continue` Skill 与「写下一章」快捷提示不再统一要求"必须 read_file 前面至少 2 章正文"，而是先用章节组细纲和进度的短期衔接提示判断本章叙事定位——常规线性推进读取最近 1-2 章核对连贯，开启新卷、新副本/新单元、单元剧或番外等相对独立的叙事单元时衔接该单元自身的起点与相关设定，而非机械读上一章。这样可以避免在这类场景下与不该衔接的旧正文强行衔接，并减少无意义的正文读取。
- Which prior prose to read before continuing is now driven by narrative structure instead of a fixed "previous two chapters": the writing system prompt, the `continue` Skill, and the "write next chapter" quick prompt no longer uniformly require reading at least the two most recent chapters. The Agent first uses the chapter-group outline and the progress short-term hooks to judge this chapter's narrative position — for normal linear progression it reads the one or two most recent chapters to verify continuity, but when opening a new volume, a new instance/arc, a standalone episode, or a side story it continues from that unit's own starting point and relevant settings rather than mechanically reading the previous chapter. This avoids forcing continuity with prose that should not be continued in those cases and reduces pointless prose reads.
- Removed low-value prompt-copy tests that only asserted fixed wording in system prompts, interactive prompts, Writing Skills, automation prompts, and SubAgent prompts via `strings.Contains` or forbidden-phrase checks. Prompt edits no longer require maintaining broad string snapshots; tests for actual message ordering, context-channel isolation, size limits, configuration parsing and merging, tool permissions, persistence, security boundaries, and end-to-end behavior remain.
- 写作 Agent 的本轮消息改为“规则与附件 → 原始用户请求”：Plan Mode、内联/按需 Writing Skill、显式文件与资料引用、编辑器选区、Review Feedback 和上下文边界统一前置，最后只保留一份带明确标题的原始请求，避免短指令被长 Skill 或引用正文埋在中间。新作品的 `CREATOR.md` 模板同时补充作品级语言约束：避免同构碎短句、连续否定、等长排比和解释性收尾形成模板化 AI 腔，并保护确实承担停顿、压迫感、人物声线或笑点的短句；`reviewer` 继续沿用原有专属提示，通过继承父 Agent system prompt 执行当前作品的 `CREATOR.md`，不在 reviewer 配置里写死这些审美规则。
- Writing Agent turn messages now follow `rules and attachments → raw user request`: Plan Mode, inline/on-demand Writing Skills, explicit file and Lore references, editor selections, Review Feedback, and the context boundary all precede one clearly titled copy of the raw request at the absolute tail, so a short instruction is not buried behind a long Skill or reference body. The new-work `CREATOR.md` template also gains work-specific prose rules against parallel sentence fragments, chained negation, equal-length lists, and explanatory endings that create a templated AI voice, while protecting short sentences that genuinely carry pauses, pressure, character voice, or humor. The reviewer keeps its existing dedicated prompt and applies the current work's `CREATOR.md` through the inherited parent system prompt rather than hard-coding these preferences in reviewer configuration.
- 前端测试按运行环境拆分：33 个纯转换/状态机测试改用轻量 Node project，其余组件、MSW、浏览器持久化与 TipTap 测试保留 jsdom，并让两个 project 顺序执行以避免 worker 过量竞争；测试 setup 使用确定性内存 `localStorage`，兼容新版 Node 的 experimental web storage 行为。重复的 preset autosave/rebase 场景从整页 `SettingPanel` 测试下沉到专门 hook 测试；真实 1 秒 autosave 等待与 Git 自动版本定时等待改为 fake timer / generation 驱动。前端全量测试墙钟由约 35.47 秒降至连续复测约 32–33 秒，且前后端均不再有单项超过 1 秒的测试。
- Frontend tests are now split by runtime: 33 pure transformation/state-machine files use a lightweight Node project, while component, MSW, browser-persistence, and TipTap tests remain in jsdom; the two projects run sequentially to avoid worker oversubscription. Test setup provides deterministic in-memory `localStorage` compatible with newer Node experimental web storage behavior. Duplicate preset autosave/rebase coverage moved from the full `SettingPanel` suite into focused hook tests, while real one-second autosave waits and Git auto-version timer waits now use fake timers or direct generation driving. Full frontend wall time dropped from about 35.47s to roughly 32–33s in consecutive runs, and neither frontend nor backend now has an individual test exceeding one second.
- `novel-standard` 的结构化审阅反馈不再一律触发一次重复的全章 reviewer：单文件内明确的错字、标点、称谓、重复、局部措辞或删除，且不改变剧情事实、动机、时间线、伏笔、世界规则、角色状态或授权范围时，由主 Agent 直接执行最小 Patch 并只验证受影响片段；意见冲突/含糊、跨文件/章节、需要扩大范围或涉及叙事事实与结构时，先 Patch，再调用一次 `reviewer` 做有界 `delta_review`。delta reviewer 只接收用户意见、修改前后片段和最小邻接上下文，只能返回 `PASS` / `BLOCKER`，不得扩展成全章审稿或提出范围外优化；`BLOCKER` 最多触发一次最小修正。新章节、完整场景和非结构化实质修订仍保留原有 `full_review`。该分流同时写入 Writing Skill、Review Feedback 本轮上下文和 reviewer SubAgent 模板，减少重复审稿模型调用并避免局部意见扩大修改范围。
- Structured review feedback under `novel-standard` no longer always triggers a redundant whole-chapter reviewer pass. Clear single-file spelling, punctuation, naming, repetition, local-wording, or deletion feedback that does not change plot facts, motivation, timeline, foreshadowing, world rules, character state, or authorized scope is applied directly as a minimal patch and validated only around the affected span. Conflicting or ambiguous feedback, cross-file/chapter impact, scope expansion, or narrative/structural changes are patched first and then receive one bounded reviewer `delta_review`. The delta reviewer receives only the user feedback, before/after spans, and minimal adjacent context; it may return only `PASS` or `BLOCKER`, must not expand into a whole-chapter review or scope-external optimization, and a blocker permits at most one minimal correction. New chapters, complete scenes, and non-structured substantive revisions keep the existing `full_review`. The routing contract is injected through the Writing Skill, per-turn Review Feedback context, and reviewer SubAgent template to reduce redundant model calls and prevent local feedback from broadening edit scope.
- 写作模式的模型上下文改为「可复用会话历史 → 当前作品上下文 → 本轮请求」：`ideas.md`、当前大纲和常驻资料不再作为易变的历史前置消息，而是与章节组、目录、进度、角色状态和 IDE 路径一起放在当前请求前，避免作品状态更新导致后续整段历史的 provider prefix cache 失效，同时让最新权威状态更靠近本轮写作任务。跨轮工具历史也改为语义投影：`read_file` 与 `skill` 的完整正文/方法只在调用当轮可见，后续仅保留可重读的 path/name receipt；历史 `grep`/`glob`/`ls` 与 `write_todos` 过程被移除；成功的 `write_file`/`edit_file`/`write_lore_items` 不再重复携带正文或 Patch 参数，只保留目标与操作数量，失败写入仍保留完整参数和错误以便恢复。完整工具卡片继续保留用于 UI 历史，不会物理删除用户会话。该变更会改变下一轮模型可见的历史工具细节，但不修改工作区内容或展示历史。
- Writing model context now follows `reusable conversation history → current workspace context → current request`. `ideas.md`, the current outline, and resident Lore no longer form a volatile message before the entire history; they now sit with chapter groups, paths, progress, character state, and IDE paths immediately before the current request. This prevents workspace edits from invalidating the provider-cache prefix for the following history while keeping authoritative state close to the active writing task. Cross-turn tool history is also projected semantically: full `read_file` and `skill` bodies remain available in the source run but become re-readable path/name receipts on later turns; historical `grep`/`glob`/`ls` snapshots and `write_todos` process state are removed; successful `write_file`/`edit_file`/`write_lore_items` calls no longer repeat prose or patch arguments and retain only target/operation metadata, while failed writes keep their full arguments and errors for recovery. Complete tool cards remain in UI history and user sessions are never physically deleted. This intentionally changes model-visible tool details on later turns without changing workspace content or display history.
- 写作模式不再把模型选择与算力档位拆成两个快捷入口；右下角统一选择器是唯一的输入框级入口，闭合状态直接显示具体模型名或当前 `Auto-*` 模式。`manual` 是新增 wire value，因此刷新或重启后仍能准确区分“指定单一模型”与行为相近但允许高级 SubAgent 覆盖的 `Auto-质量优先`。
- Writing Mode no longer exposes model selection and compute tier as two separate quick controls. The unified bottom-right selector is the only composer-level entry and its closed state displays either the concrete model name or the active `Auto-*` mode. `manual` is a distinct wire value, so refreshes and restarts preserve the difference between a forced single model and behaviorally similar `Auto-Quality`, which still permits advanced SubAgent overrides.
- 写作 SubAgent 的默认算力从「全部继承主 Agent 的 pro + 思考」改为随「写作算力档位」按阶段分配，默认档位为 `balanced`：这意味着开箱行为发生变化——`reviewer`、`context-planner`、`final-gate`、`memory-patcher` 以及两个 choreographer 现在默认走 `flash`（推理阶段保留思考、检查阶段关闭思考），而决定成稿质量的 `writer`、`fixer` 仍走 `pro`。如需完全恢复旧行为（全部 pro + 思考），将档位切到 `quality` 即可。该变更仅影响写作模式管线。
- Writing sub-agents no longer all inherit the main agent's pro + thinking by default; their compute is now allocated by stage via the "Writing Compute Tier", defaulting to `balanced`. This changes out-of-the-box behavior: `reviewer`, `context-planner`, `final-gate`, `memory-patcher`, and the two choreographers now default to `flash` (thinking kept for reasoning stages, off for check ones), while quality-critical `writer` and `fixer` stay on `pro`. To fully restore the previous behavior (all pro + thinking), switch the tier to `quality`. This change affects the Writing Mode pipeline only.
- `novel-standard` / `novel-heavy` 现按场景复杂度使用 choreography：普通对白、静态描写、简单动作和单次亲密接触不增加额外模型往返；复杂战斗/追逐/多人协作可调用一次 `choreographer`，复杂亲密/情色互动可调用一次 `intimacy-choreographer`。`novel-standard` 在主 Agent 初稿前按需编排且每场景最多一次；`novel-heavy` 由 Context Plan 先判定，再把可选 beat sheet 交给 writer。亲密场景的含蓄/直白尺度必须从用户原始要求原样传递，choreography 不得自行升级或净化。`novel-lite` 继续保持低延迟，不调用任何 SubAgent。
- `novel-standard` and `novel-heavy` now use choreography based on scene complexity. Ordinary dialogue, static description, simple actions, and one-off intimate contact add no extra model round trip; complex combat, pursuit, or multi-party cooperation may call `choreographer` once, while complex intimate or erotic interaction may call `intimacy-choreographer` once. `novel-standard` choreographs only when needed before the main-agent draft and at most once per scene; `novel-heavy` lets the Context Plan decide first and passes an optional beat sheet to the writer. The user's requested restrained/explicit scale must be forwarded unchanged, and choreography must neither escalate nor sanitize it. `novel-lite` remains low-latency and uses no SubAgents.
- Writing preset 的直接加载改为小模型语义路由：内置快捷创作仍可在代码中携带可信内部 `intent`（规划、正文创作、正文修订、应用审阅意见、只读检查），结构化审阅和斜杠命令也直接路由；除此之外的每条自由输入统一使用 `writing_compute_fast_profile_id` 指向的 fast profile 做一次无历史、thinking off 的意图识别，不再依赖大段写作关键词规则。写作输入框和自定义快捷按钮设置均删除「自动 / 规划 / 正文创作 / 正文修订 / …」手动选择器，自定义按钮统一自动识别。分类器输出 `discussion / planning / prose_generation / prose_revision`、`execute_now`、置信度和当前消息原文证据；只有 `execute_now=true`、高置信、正文创作/修订且证据逐字存在于本轮消息时才允许加载 Writing preset，分类失败、低置信或证据无效一律回退为讨论。“不要写 / 先讨论”等明确否定仍由本地硬规则直接拦截。内置 preset 直接注入仍有 128K 字符硬上限，日志与 run trace 会记录最终 `writing_intent`、inline/dynamic 路径及判定原因。
- Direct Writing-preset loading now uses small-model semantic routing. Built-in Quick Writing actions may still carry trusted internal `intent` metadata in code (planning, prose generation, prose revision, review application, or read-only analysis), while structured review feedback and slash commands route directly. Every other free-form message is classified once through the history-free, thinking-off model selected by `writing_compute_fast_profile_id`, replacing the large writing-keyword router. The low-value `Auto / Planning / Write Prose / Revise Prose / …` selector has been removed from both the composer and custom Quick Writing settings, and custom actions always use automatic classification. The classifier returns `discussion / planning / prose_generation / prose_revision`, `execute_now`, confidence, and verbatim evidence from the current message. A Writing preset loads only when `execute_now=true`, confidence is high, the intent is prose generation/revision, and the evidence occurs verbatim in the current message; failures, lower confidence, or invalid evidence always fall back to discussion. Explicit negatives such as “do not write” or “discuss first” remain local hard guards. Inline built-in presets retain the 128K-character hard limit, and logs/run traces record the final `writing_intent`, inline/dynamic path, and decision reason.
- `action-choreography` 与 `intimacy-choreography` 现在通过 Skill frontmatter 的 `delegate` 路由到专业只读 SubAgent：专业 Agent 可用时，父 Agent 加载 Skill 只收到精简的 `task` 委派协议，完整 3K+ 方法仅在 `choreographer` / `intimacy-choreographer` 独立上下文中加载，避免父上下文污染，并解除游戏模式“只能输出正文”与 beat sheet 输出协议的冲突；专业 Agent 被关闭或对当前父级不可用时，Skill 会明确返回 BLOCKED 并提示在 Agents 页启用，不再静默由父 Agent执行重方法。两个专业 SubAgent 已移入 Go 内置默认，不再依赖发布包 `config.toml` 才存在；Agents 页仍可按稳定 ID 覆盖、改名或关闭。
- `action-choreography` and `intimacy-choreography` now route through a `delegate` field in Skill frontmatter to dedicated read-only SubAgents. When the specialist is available, the parent Agent receives only a compact `task` delegation contract and the full 3K+ method loads only inside the isolated `choreographer` / `intimacy-choreographer` context, preventing parent-context pollution and avoiding the Game Mode conflict between narrative-only output and beat-sheet output. If the specialist is disabled or unavailable to the current parent, the Skill returns an explicit BLOCKED result and directs the user to enable it in Agents instead of silently executing the heavy method in the parent. Both specialists are now built-in Go defaults instead of depending on the packaged `config.toml`; they remain overridable, renameable, and disableable by stable ID from the Agents page.
- 「快捷创作」按钮不再点击后立即发送消息；现在会把提示词填入 Agent 编辑框并聚焦，作者可继续修改、补充引用或切换输入选项后再手动发送。输入框工具栏新增常驻「快捷创作」入口，即使当前会话已经产生对话仍可展开全部动作并沿用当前上下文；如需隔离任务，仍由作者显式新建会话。快捷区与常驻菜单均可直达设置，空按钮列表也会引导用户前往配置。
- Quick Writing actions no longer send a message immediately. Clicking one now fills and focuses the Agent composer so the author can edit it, add references, or adjust composer options before sending manually. A persistent Quick Writing menu now lives in the composer toolbar, keeping every action available after a conversation already has messages and reusing the current context; authors can still explicitly start a new conversation when isolation is needed. Both Quick Writing surfaces link directly to Settings, and an empty action list guides users to configure it.
- 内置 `novel-lite` / `novel-standard` / `novel-heavy` 在明确的正文创作、续写、修订或结构化审阅反馈回合中改为由后端直接解析并注入本轮上下文，省去模型先调用 `skill` 工具再继续写作的一次往返；普通问答/分析、显式斜杠 Skill、自定义 Skill，以及用户或工作区对同名内置 Skill 的覆盖仍保留动态加载，扩展优先级不变。
- Built-in `novel-lite` / `novel-standard` / `novel-heavy` presets are now resolved and injected directly for clear prose generation, continuation, revision, and structured review-feedback turns, removing the model's extra `skill` tool round trip. General Q&A/analysis, explicit slash Skills, custom Skills, and user/workspace overrides of the same built-in names remain dynamically loaded, preserving extension precedence.
- 三个内置 Writing preset 已去除由 system prompt 统一承担的重复文件工具与 Patch 规则，只保留各自工作流差异。`novel-standard` 现在把用户提交的结构化审阅意见视为 reviewer 的重点输入，而不是独立审稿的替代品：所有正式写作任务都必须委派一次 reviewer；reviewer 结合最新正文核对评论有效性、冲突、最小影响范围和连续性风险后，由主 Agent 聚合意见并修订，默认不在修订后机械追加第二轮 reviewer。
- The three built-in Writing presets now omit file-tool and patch rules already enforced by the system prompt and keep only their workflow differences. `novel-standard` now treats submitted structured review feedback as priority input for the reviewer rather than a replacement for independent review: every formal writing task must delegate to one reviewer, which checks the latest prose for comment validity, conflicts, minimal impact scope, and continuity risks before the main Agent aggregates the findings and revises. A second mechanical reviewer pass is not added after revision by default.
- 三档 Writing preset 现在严格分离审稿职责，避免父 Agent 与专业 SubAgent 重复审阅：`novel-lite` 因不使用 reviewer，仅保留一次有界轻量自检和最多一次最小修正；`novel-standard` 在初稿落盘后立即委派 reviewer，reviewer 返回前禁止父 Agent 全文读回、grep、自列问题或修订，随后只做一次统一 Patch 与最终机械验证；`novel-heavy` 强制 `writer -> reviewer -> fixer -> final-gate -> memory-patcher` 直接衔接，父 Agent 不得在专业阶段间插入第二套审稿，只有 final-gate 的 blocker 可触发一次 fixer 返工。system、Creator 模板、章节续写 Skill、快捷写作与自动化提示均改为服从当前 Writing Skill 的阶段顺序，不再以通用“正文自检”诱发预审。
- The three Writing presets now separate review ownership to prevent duplicate parent-Agent and specialist passes. `novel-lite`, which has no reviewer, keeps one bounded lightweight check and at most one minimal correction. `novel-standard` delegates to the reviewer immediately after the draft is written, forbids parent rereads, grep checks, issue lists, or revisions before the reviewer returns, then performs one consolidated patch followed by mechanical verification. `novel-heavy` enforces direct `writer -> reviewer -> fixer -> final-gate -> memory-patcher` handoffs with no parallel parent review; only a final-gate blocker may trigger one fixer retry. System, Creator-template, chapter-continuation Skill, Quick Writing, and automation prompts now defer to the active Writing Skill's stage order instead of inducing a generic pre-review self-check.
- 章节修订与审阅反馈统一改为「最小必要 Patch」协议：系统提示、`rewrite`、`novel-lite`、`novel-standard`、`novel-heavy` 和默认 fixer SubAgent 现在都要求先完整解决用户/审阅意见，再保护未涉及原文、强段落、人物声线、情节节点、伏笔和连续性；评论数量不会自动扩大修改范围。已有章节默认使用一次批量 `edit_file` 定点修改，只有作者明确要求整章/全文/换视角/彻底改写或整体重构时才允许 `write_file` 覆盖。
- Chapter revision and review feedback now follow one "minimal necessary patch" contract: the system prompt, `rewrite`, `novel-lite`, `novel-standard`, `novel-heavy`, and the default fixer SubAgent must fully address the author's/reviewer's requirements while preserving unaffected prose, strong passages, character voice, plot beats, foreshadowing, and continuity; comment count never broadens edit scope. Existing chapters default to one batched `edit_file` patch and may be overwritten with `write_file` only when the author explicitly requests a full-chapter/full-text/POV/complete rewrite or structural reconstruction.
- `edit_file` 的模型说明新增精确匹配恢复流程：修改前必须重新读取最新文件并逐字复制 `old_string`（包括中英文标点、引号、空格与换行）；遇到未找到、不唯一或 revision 冲突时，重新读取并构造更小且唯一的 patch，不再自动降级为整文件覆盖。
- The model-facing `edit_file` guidance now includes a safe exact-match recovery flow: re-read the latest file and copy `old_string` verbatim, including punctuation, quotes, spaces, and line breaks; when matching is missing, ambiguous, or revision-conflicted, re-read and build a smaller unique patch instead of falling back to a full-file replacement.
...
- 修复自定义 SubAgent 因提示词含字面花括号而被整体废弃的 bug：当运行中存在 SessionValues（例如本轮调用过 eino 内置 `write_todos` 工具）时，父 Agent 通过 `task` 调用自定义 SubAgent 会走 eino 默认的 GenModelInput，把 SubAgent instruction 当 FString 模板解析；提示词里的 JSON 示例等字面 `{order}` 花括号会触发 "could not find key" 错误并直接废掉本次子 Agent 调用。现在自定义 SubAgent 统一传入不做任何模板插值的 GenModelInput，instruction 原样透传给模型。该修复无功能回退：Denova 全仓库不使用 eino 的 instruction FString 插值（动态上下文在 prompt 组装层完成），且 eino deep 包内置的 root/general agent 本就不做该格式化，修复前后模型可见行为一致；唯一变化是原本会报错废弃的调用现在正常执行。
- Fixed a bug where a custom SubAgent call could be wasted entirely because its prompt contained literal curly braces: when SessionValues existed mid-run (e.g. after the built-in `write_todos` tool fired), invoking a custom SubAgent via `task` went through eino's default GenModelInput, which parsed the SubAgent instruction as an FString template; literal braces such as `{order}` inside JSON examples then raised "could not find key" and aborted that sub-agent invocation. Custom SubAgents now pass a no-formatting GenModelInput so the instruction reaches the model verbatim. No functionality is lost: Denova never uses eino's instruction FString interpolation (dynamic context is assembled before eino), and eino's own deep-package root/general agents never applied this formatting either, so model-visible behavior is unchanged — the only difference is that calls which previously failed now succeed.
- 修复写作模式中 reviewer 审稿期间前端偶发内存溢出：SubAgent 尚未输出最终结论时，主聊天卡会用流式 thinking 生成 220 字预览，旧实现却先把完整内容展开成 Unicode 字符数组，长推理会造成远超原文大小的瞬时内存分配。现在预览、计划上下文、正文评论锚点与风格文本上限统一使用有界 Unicode 切片，只扫描并分配实际需要的前缀或后缀，同时保持代理对、截断语义和完整 SubAgent 详情不变；写作与游戏模式的共通文本边界一起受保护。
- Fixed intermittent frontend out-of-memory failures while reviewer runs in Writing Mode. Before the SubAgent emitted its final result, the main chat card built a 220-character preview from streaming thinking by first expanding the entire output into a Unicode character array, causing transient allocations far larger than long reviews themselves. Previews, plan context, document-review anchors, and style-source limits now share bounded Unicode slicing that scans and allocates only the required prefix or suffix, while preserving surrogate pairs, truncation semantics, and full SubAgent details. The shared text boundaries protect both Writing and Interactive modes.
- Agent 输出现在会显示本次实际使用的模型：主 Agent 消息底部展示解析后的 `model_name` 与 profile，SubAgent 输出卡片和子会话标题也展示各自真实模型；模型身份在运行开始时按同一配置快照解析，经 SSE 与会话历史持久化，不会因运行中或之后切换设置而把旧消息标成新模型。该信息仅作为展示元数据保存，不进入下一轮模型上下文。旧历史没有该字段时保持原样，不显示模型标签。
- Agent output now shows the model actually used for that run. Main Agent messages display the resolved `model_name` and profile in their footer, while SubAgent output cards and sub-session headers display each SubAgent’s own resolved model. Model identity is resolved from the same configuration snapshot at run start and persists through SSE and session history, so changing settings during or after a run cannot relabel older output with a newer model. This is display-only metadata and never enters subsequent model context. Older history without these fields remains unchanged and simply shows no model label.
- 修复渲染几万字的已完成消息时前端卡死：根因是一条长消息在单次提交里同步挂载数百个 Markdown 块（每块一次完整解析），以及流式结束时把整棵渲染树换成另一组件类型导致全量重挂重解析。现将「流式」与「已完成」统一为同一 `MarkdownContent`（同组件、同 key 序列），流式结束不再重挂重解析；同时超过块数阈值的已完成大段内容改为分帧渐进挂载，渲染未完成时在末尾显示「正在渲染…」指示（中英双语），全程主线程不再卡死，滚动与交互保持可用。写作与游戏模式及 SubAgent 输出共用同一渲染路径。
- Fixed the frontend freezing when rendering a completed message of tens of thousands of characters: the root causes were mounting hundreds of Markdown blocks (each fully parsed) in a single synchronous commit, and swapping the whole render tree to a different component type at stream end, which remounted and re-parsed everything. Streaming and completed states now share one `MarkdownContent` (same component, same key sequence), so stream end no longer remounts/re-parses; large completed content above a block-count threshold is revealed progressively across animation frames, showing a bilingual "Rendering..." indicator at the tail until done. The main thread no longer freezes and scrolling/interaction stay responsive. Shared by Writing and Interactive modes and SubAgent output.
- 缓解打开超大章节（数万字以上）时编辑器卡顿的观感：章节编辑器基于 TipTap/ProseMirror，不做虚拟化，会一次性同步解析并渲染整篇正文，超大章节挂载会阻塞主线程数秒、像卡死。现新增挂载前的加载门 `DeferredMarkdownEditor`，超过字符阈值（约 5 万字）的文档先绘制「正在打开章节…」加载指示（中英双语，并附「建议拆分章节」的提示）再挂载重编辑器，把黑屏假死变成明确的加载态；同时超大文档跳过需要全文遍历的对白高亮装饰，减轻解析压力。普通大小章节行为完全不变、立即挂载。注意这是观感缓解而非彻底消除：那一帧的同步解析仍会占用主线程，超长单章仍建议拆分。
- Improved the perceived responsiveness when opening very large chapters (tens of thousands of characters and up): the chapter editor is built on TipTap/ProseMirror, which is not virtualized and synchronously parses and renders the whole body at once, blocking the main thread for seconds on huge chapters so it looks frozen. A pre-mount gate `DeferredMarkdownEditor` now paints a bilingual "Opening chapter…" loading indicator (with a "consider splitting the chapter" hint) before mounting the heavy editor for documents above a character threshold (~50k), turning the dead-looking blank screen into a clear loading state; very large documents also skip the whole-document dialogue-highlight decoration to reduce parse cost. Normal-sized chapters are unchanged and mount immediately. Note this mitigates the perception rather than eliminating the freeze — that frame's synchronous parse still occupies the main thread, and very long single chapters are still best split.

- 修复前端构建/开发时 Node.js 输出的 `[DEP0205] DeprecationWarning: `module.register()` is deprecated. Use `module.registerHooks()` instead.` 警告：该告警来自传递依赖 `@tailwindcss/node@4.3.0`（经 `@tailwindcss/vite` 引入）在 ESM 缓存 loader 注册时调用了已废弃的 `module.register()`。将 `@tailwindcss/vite` 与 `tailwindcss` 升级到 `4.3.3` 后，`@tailwindcss/node` 会在 Node 支持时优先使用 `module.registerHooks()`，告警消除。
- Fixed the `[DEP0205] DeprecationWarning: `module.register()` is deprecated. Use `module.registerHooks()` instead.` warning emitted by Node.js during frontend build/dev. It originated from the transitive dependency `@tailwindcss/node@4.3.0` (pulled in via `@tailwindcss/vite`), whose ESM cache-loader registration called the deprecated `module.register()`. Bumping `@tailwindcss/vite` and `tailwindcss` to `4.3.3` lets `@tailwindcss/node` prefer `module.registerHooks()` when Node supports it, removing the warning.

...
- 大纲与细纲结构模板改为用户可配置（设置 → 写作模式，新增「大纲结构模板」「细纲结构模板」，对应 `outline_format` / `chapter_group_format`，workspace 级、留空即用系统内置默认）：此前这两种结构写死在 `outline` 与 `group-plan` 两个 Skill 里、用户无法修改，现在统一由写作系统提示的「大纲与细纲结构」段注入，两个 Skill 改为遵循注入结构而不再各写一套。模板作为每轮注入的系统提示文本，设有约 8000 字硬上限，超出在保存时截断。写作与游戏模式共用同一注入路径。
- Outline and chapter-group structure templates are now user-configurable (Settings → Writing Mode adds "Outline Structure Template" and "Chapter-Group Structure Template", backed by `outline_format` / `chapter_group_format`, workspace-scoped, empty means the built-in default). These structures were previously hardcoded inside the `outline` and `group-plan` Skills and could not be changed; they are now injected once through the "outline and chapter-group structure" section of the writing system prompt, and both Skills defer to the injected structure instead of each defining their own. The template is injected system-prompt text with a hard cap of about 8000 characters, truncated on save. Writing and Interactive modes share the same injection path.

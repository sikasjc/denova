...
- Writing preset 的直接加载不再只依赖后端关键词猜测：快捷创作配置新增可选 `intent`（规划、正文创作、正文修订、应用审阅意见、只读检查），内置六项会携带明确 intent 到聊天与上下文分析接口；当前 intent 会在写作 Agent 输入框工具栏中可见且可在发送前修改，但不会写入用户发送文本。自由输入和未配置 intent 的自定义按钮仍保留保守 fallback。内置 preset 直接注入增加 128K 字符硬上限，超限自动退回动态 Skill 加载；日志与 run trace 会记录 `writing_intent`、inline/dynamic 路径及 fallback 原因。
- Direct loading of Writing presets no longer depends only on backend keyword guessing. Quick Writing actions now support an optional `intent` (planning, prose generation, prose revision, review application, or read-only analysis), and the six built-ins carry an explicit intent through both chat and context-analysis requests. The current intent is visible and editable in the Writing Agent composer before sending, but is never inserted into the user's message text. Free-form input and custom actions without an intent retain the conservative fallback. Inline built-in presets now have a hard 128K-character limit and fall back to dynamic Skill loading when exceeded, while logs and run traces record `writing_intent`, inline/dynamic delivery, and fallback reasons.
...
- 三个内置 Writing preset 已去除由 system prompt 统一承担的重复文件工具与 Patch 规则，只保留各自工作流差异；体积约从 4.0/5.2/7.4 KB 降至 1.9/2.6/5.9 KB。`novel-standard` 在用户已经提交结构化审阅意见时直接聚合并应用 Patch，除非用户要求复审或存在明显连续性/结构风险，否则不再重复启动 reviewer。
- The three built-in Writing presets now omit file-tool and patch rules already enforced by the system prompt and keep only their workflow differences, reducing their size from roughly 4.0/5.2/7.4 KB to 1.9/2.6/5.9 KB. `novel-standard` now treats submitted structured review feedback as the review result and applies the patch directly unless the user requests another review or clear continuity/structural risk requires one.

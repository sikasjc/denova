...
- 新增「写作算力档位」（`writing_compute_tier`）：一个开关按写作管线阶段整体切换各 SubAgent 的模型与思考强度，在质量与速度/成本之间平衡。三档为 `quality`（全部阶段 pro + 思考，等价历史行为）、`balanced`（默认；正文 writer/fixer 用 pro，推理 reviewer/规划/编排与检查 门禁/状态补丁下放到 flash）、`speed`（全部 flash，仅正文保留思考）。档位按 SubAgent 的 `compute_role`（`prose`/`reasoning`/`mechanical`，展示名分别为正文/推理/检查）而非固定 ID 映射，是介于「父 Agent 继承」与「逐 SubAgent 显式覆盖」之间的一层：不动任何 SubAgent 即可一键切档，而任何显式 `[sub_agents.model]` 覆盖仍优先于档位。档位只作用于写作模式，游戏模式的 choreographer 不受影响。写作模式编辑框新增「算力档位」选择器并展示每档各阶段实际使用的模型/思考；Agents 页（写作 → 用户层）新增档位选择器与「各阶段生效模型」只读汇总，且每个写作 SubAgent 编辑器新增「算力角色」选择，其「继承」模型会随档位实时反映。内置六个写作 SubAgent 与两个 choreographer 已带默认 `compute_role`。中英文完整本地化。
- Added a "Writing Compute Tier" (`writing_compute_tier`): one switch that changes the model and thinking strength of every writing-pipeline sub-agent by stage, balancing quality against speed/cost. The three tiers are `quality` (all stages pro + thinking, equivalent to the historical behavior), `balanced` (default; prose writer/fixer stay pro while reasoning reviewer/planner/choreography and check gate/memory-patch move to flash), and `speed` (all flash, thinking kept only for prose). The tier maps by each sub-agent's `compute_role` (`prose`/`reasoning`/`mechanical`, shown as Prose/Reasoning/Check) rather than fixed IDs, sitting between "parent-agent inheritance" and "explicit per-sub-agent override": you can switch tiers without touching any sub-agent, while any explicit `[sub_agents.model]` override still wins over the tier. The tier applies to Writing Mode only; Interactive Mode choreographers are unaffected. The Writing composer gains a "Compute Tier" selector that shows the resolved model/thinking per stage; the Agents page (Writing → user layer) gains a tier selector plus a read-only "effective model per stage" summary, and each writing sub-agent editor gains a "Compute Role" selector whose "inherited" model reflects the tier live. The six built-in writing sub-agents and the two choreographers ship with default `compute_role`s. Fully localized in Chinese and English.
...
- 档位的「快速模型」现在可自选（`writing_compute_fast_profile_id`，默认 `flash`）：Agents 页（写作 → 用户层）算力档位区新增「快速模型」下拉，可把 `balanced` / `speed` 档下放阶段指向任意已配置 profile；选择即时反映到「各阶段生效模型」汇总与每个写作 SubAgent 编辑器的「继承」值，保存后写作编辑框档位提示同步更新。所选 profile 不存在时仍回退到 `default`（pro）安全降级。
- The tier's "fast model" is now selectable (`writing_compute_fast_profile_id`, default `flash`): the Agents page (Writing → user layer) compute-tier section adds a "Fast Model" dropdown that points the offloaded stages of `balanced` / `speed` at any configured profile. The choice is reflected immediately in the "effective model per stage" summary and each writing sub-agent editor's "inherited" value, and after saving the Writing composer's tier hint updates too. If the chosen profile does not exist, it still safely falls back to `default` (pro).

...
- 写作 Agent 的每轮作品上下文改为有界、按需检索的快照：稳定上下文最多保留灵感 3 KiB、大纲 5 KiB 和常驻资料目录 4 KiB；动态上下文最多保留最新一个章节组 8 KiB、章节路径 2 KiB 与角色状态 10 KiB，两个通道总计分别限制为 12 KiB / 20 KiB 的源内容。常驻资料目录提供 `id`、名称、类型和重要度，模型需要角色或设定正文时必须通过既有 `read_lore_items` 按需读取，不再把所有常驻资料正文默认注入每个请求。完整文件仍可通过 `read_file` 读取；工作区与展示历史均不受影响。这是 Beta 阶段有意的模型可见上下文行为变更。
- The Writing Agent's per-turn workspace context is now a bounded, retrieval-first snapshot: stable source content retains at most 3 KiB of ideas, 5 KiB of outline, and a 4 KiB resident-Lore index; dynamic source content retains at most the newest chapter group (8 KiB), chapter paths (2 KiB), and character state (10 KiB), with 12 KiB / 20 KiB channel budgets. The resident-Lore index provides each item's `id`, name, type, and importance; when a character or setting body is needed, the model must load it through the existing `read_lore_items` tool instead of receiving every resident body on every request. Full files remain available through `read_file`, and neither the workspace nor displayed history is changed. This is an intentional Beta change to model-visible context behavior.
- Run traces now record metadata for the stable and dynamic workspace fragments that remain in the final post-compaction model messages, so context-source byte counts and inclusion state can be audited against provider token usage without persisting the fragment bodies.
- 写作 Agent 的 system prompt 现准确区分“已注入的有界大纲/状态快照”和“仅提供目录、必须按需读取正文的资料库”，避免模型误把角色与设定正文当作已知内容。与此同时，`task`、`write_todos` 与 `skill` 保持原有调用能力和参数契约，但改用有界的 Denova 专用说明，移除上游面向通用编码 Agent 的重复教程；`write_todos` 的成功返回由回显完整待办 JSON 改为简短数量确认。
- The Writing Agent system prompt now accurately distinguishes bounded injected outline/state snapshots from the Lore index whose bodies must be read on demand, preventing the model from treating character and setting bodies as already known. `task`, `write_todos`, and `skill` retain their existing capabilities and parameter contracts but now use bounded Denova-specific guidance instead of repeated upstream generic coding tutorials; a successful `write_todos` call now returns a short count confirmation instead of echoing the full Todo JSON.
- 写作文件工具已从模型可见的 `edit_file` 迁移为 `replace_lines` 与 `replace_text`（Beta 阶段不兼容变更）。`replace_lines` 只接受同一 revision 快照中的完整行范围，`content` 必传且仅显式空字符串表示删除；`replace_text` 用于人名、术语等字面量批量替换，直接对调用时文件的当前内容替换全部命中，不依赖 revision 或预先统计命中数。`write_file` 继续仅用于新文件或明确全量重写；禁用文件写入时不再把写工具 schema 注入模型。
- Model-visible writing file tools have migrated from `edit_file` to `replace_lines` and `replace_text` (an intentional Beta breaking change). `replace_lines` accepts only complete line ranges from one revision-checked snapshot, with required `content` and deletion only through an explicit empty string. `replace_text` is for literal bulk changes such as names and terminology: it replaces every match in the current file snapshot without requiring a revision or pre-counted occurrences. `write_file` remains for new files or explicitly authorized full rewrites; when file writing is disabled, write-tool schemas are no longer injected into the model.
- 写作 Agent 默认不再向模型暴露 `glob` 或 `execute`：`ls`、`read_file` 与 `grep` 已覆盖作品目录浏览、按路径读取和正文检索，避免 `glob` 的重复 schema；任意 Shell 执行默认关闭，只有显式开启 `shell_execute` 高级能力时才会重新提供。默认模型工具数量因此从 17 降至 15。
- The Writing Agent no longer exposes `glob` or `execute` by default: `ls`, `read_file`, and `grep` cover manuscript browsing, path reads, and prose search without `glob`'s overlapping schema. Arbitrary shell execution is off by default and returns only when the advanced `shell_execute` capability is explicitly enabled. The default model-visible tool count drops from 17 to 15.
...
- 修复 `edit_file` 因 revision 格式不一致而误报冲突的问题：revision 统一为不带 `sha256:` 前缀的 64 位裸 hex，并由 `internal/revisionfile` 作为唯一生成实现，`workspacechange` 与 `book` 复用同一口径。不兼容旧前缀格式；传入值与当前值不完全一致时按真实冲突处理。
- `edit_file` / `write_file` 的 revision 冲突报错改为精简中文：`Revision 冲突：传入 <expected>，当前 <actual>。`，结构化 `details` 同时保留 `expected_revision` 与 `actual_revision`，便于模型直接比较。
- `read_file` 新增 `revision_only: true`：只返回路径、当前 revision 与文件字节数，不返回正文，适合冲突后仅刷新 revision；正常连续编辑继续优先使用 `edit_file` / `write_file` 成功回执中的新 revision。
- `edit_file` 工具说明明确一次调用批量提交同一文件的多处修改：`edits` 是数组，所有选择器基于同一原始快照解析，整批只推进一次 revision；后续调用必须使用成功回执返回的新 revision。

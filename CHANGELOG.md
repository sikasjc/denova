...
- 新增内置 Skill `action-choreography`（多主体动作编排）：把一句简单场景描述编排成结构化的「动作节拍表(beat sheet)」，只产出节拍表、不负责扩写正文，扩写交给下游 Agent 或后续回合据表完成。核心抽象是「主体 + 动作/状态」——主体不限于人（可为法术造物、军阵单位、载具、自然之力、环境本身），动作可为瞬时动作或跨多拍的持续状态；每一拍强制守住空间连续、状态连续、反应因果三条约束，并内置防瞬移/防幽灵资源/防全知反应等护栏。覆盖对抗（剑术、魔法对轰、室内枪战）、协作（双人攀登、军阵演练）、过程与灾难（陨石撞击、塌方蔓延）等场景，`agent: ide,interactive_story,choreographer` 使写作与游戏两个模式，以及编排 SubAgent 均可加载。
- Added a built-in `action-choreography` Skill (multi-subject action choreography): it turns a short scene description into a structured "beat sheet" and only produces the beat sheet, leaving prose expansion to a downstream Agent or later turns. Its core abstraction is "subject + action/state" — subjects are not limited to people (spells, formation units, vehicles, natural forces, or the environment itself all qualify) and actions can be instantaneous or a multi-beat sustained state; every beat must preserve spatial continuity, state continuity, and reaction causality, with built-in guardrails against teleporting, phantom resources, and omniscient reactions. It covers combat (swordplay, magic duels, indoor gunfights), cooperation (two-person climbing, formation drills), and processes/disasters (meteor impact, spreading collapse); `agent: ide,interactive_story,choreographer` lets both Writing and Interactive modes and the choreographer SubAgent load it.
- 新增预置 SubAgent `choreographer`（动作编排）：`parents = ["ide", "interactive_story"]` 使写作与游戏模式都能通过 `task` 委派它。它在独立上下文里加载 `action-choreography` Skill 并完成立舞台、逐主体状态、逐拍推演与连续性自检，只把紧凑节拍表返回父 Agent，避免大量编排推演污染父 Agent 上下文。因只产出节拍表、不扩写、不落盘，其工具经 `[sub_agents.tools]` 显式收窄为只读（关闭 `file_write`/`lore_write`/`todo`/`web_search`/`image_generation`/`shell_execute`，保留 `file_read` 取连续性依据、`skills` 加载方法论），即使在允许写文件的写作父 Agent 下也不会落盘。可在 Agents 页像其他预置写作 SubAgent 一样改名、改提示词或关闭。
- Added a preseeded `choreographer` SubAgent (action choreography): `parents = ["ide", "interactive_story"]` lets both Writing and Interactive modes delegate to it via `task`. It loads the `action-choreography` Skill in an isolated context and performs stage setup, per-subject state tracking, beat-by-beat reasoning, and continuity self-checks there, returning only the compact beat sheet so the heavy choreography reasoning never pollutes the parent Agent's context. Because it only produces a beat sheet (no prose, no writes), its tools are explicitly narrowed to read-only via `[sub_agents.tools]` (disabling `file_write`/`lore_write`/`todo`/`web_search`/`image_generation`/`shell_execute`, keeping `file_read` for continuity grounding and `skills` to load the method), so it never writes files even under the write-capable Writing parent. It can be renamed, re-prompted, or disabled from the Agents page like the other preseeded writing SubAgents.
- 新增内置 Skill `intimacy-choreography`（亲密动作编排）与预置 SubAgent `intimacy-choreographer`：与动作编排同一套引擎（STAGE / SUBJECTS / BEATS + 连续性校验），但把字段侧重换到肢体、情绪、回应与张力（`意愿`、`张力`、`情绪·氛围` 通道等），并新增「意愿连续」校验——任一方给出迟疑/退缩/暂停信号时后续拍必须先回应。本 Skill 只做编排、不判断也不控制描写尺度：露骨程度、含蓄或直白，完全由主 Agent 提供的场景描述承载、由下游扩写最终呈现，Skill 既不自行升级也不自行净化，只忠实反映输入中已表达的内容与走向；无论输入偏含蓄还是直白，产物始终停留在节拍表抽象层，不输出正文/台词/感官描写。所有主体默认成年且自愿，冲突或不明确时输出 `[BLOCKER]` 请主 Agent 确认。SubAgent `parents = ["ide", "interactive_story"]`、工具收窄为只读，可在 Agents 页改名/改提示词/关闭。
- Added a built-in `intimacy-choreography` Skill and a preseeded `intimacy-choreographer` SubAgent: they reuse the same engine as action choreography (STAGE / SUBJECTS / BEATS + continuity checks) but shift the fields toward body language, emotion, response, and tension (`意愿`/consent-signal, `张力`/tension, an `情绪·氛围` channel, etc.) and add a "consent continuity" check — when any subject signals hesitation, withdrawal, or a pause, the following beats must address that signal first. The Skill only choreographs and does not judge or control explicitness: how restrained or explicit the scene is comes entirely from the main Agent's scene description and the downstream expansion, and the Skill neither escalates nor sanitizes — it faithfully reflects what the input already expresses. Regardless of whether the input leans restrained or explicit, the output stays at the beat-sheet abstraction level and never emits prose, dialogue, or sensory description. All subjects are assumed adult and consenting; on conflict or ambiguity it emits `[BLOCKER]` for the main Agent to confirm. The SubAgent uses `parents = ["ide", "interactive_story"]` with read-only tools and can be renamed, re-prompted, or disabled from the Agents page.
...
- 清理低价值的提示词文案单测：删除仅通过 `strings.Contains` / forbidden phrase 校验 System Prompt、互动 Prompt、Writing Skill、自动化 Prompt 和 SubAgent Prompt 固定措辞的测试，不再因提示词编辑而维护大批字符串快照。仍保留真实消息装配顺序、上下文 channel 隔离、大小上限、配置解析与合并、工具权限、持久化、安全边界和端到端行为测试。

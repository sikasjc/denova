...
func editableAutomationBuiltinInstruction(cfg *config.Config, state *book.State, task AutomationTaskInstruction) string {
	...
	sb.WriteString("- 如果任务需要续写章节，先检查 `setting/outline.md`、`setting/chapter-groups/`、`setting/character-states.md`、最近实际章节和资料库，以实际章节路径与非空正文决定目标章节；本轮有生效 Writing preset 时严格按其审稿、修订与最终机械验证顺序执行，否则只做一次轻量自检和最多一次最小修正，不额外增加审稿流水线；形成最终稿后在同一轮同步角色状态，章节状态标签不构成同步门槛。\n")

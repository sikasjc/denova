...
const reviewFeedbackPrefix = "\n\n# Review feedback / 审阅反馈\n\n" +
	...
	"Apply review feedback as a minimal necessary patch, not as permission to regenerate the chapter. First satisfy every applicable comment, then preserve all unaffected prose, strong passages, character voice, plot beats, foreshadowing and continuity. The number of comments does not broaden the authorized edit scope and does not by itself make the task high risk. Group comments by file, make a compact internal patch plan, and combine non-overlapping changes to one file into one edit_file call. Use the latest numbered body already in your context as the working snapshot (cross-turn assembly keeps it current, including a refreshed body after your own edits); only read a file first when its current numbered body is missing from your context, and then copy its revision to file_revision. Use the returned 1-based line numbers with start_line/end_line; use an exact old_string only when the change cannot be expressed as a line range (a partial-line edit inside one line) or replace_all applies. If a line range is stale or exact matching fails, rebuild the selector from the current numbered body in context; only a reported revision conflict requires re-reading. Never fall back to write_file for an existing chapter unless the user explicitly requested a full-chapter rewrite. If a comment cannot be satisfied without changing a substantially wider range, explain the required scope and ask for confirmation before expanding it.\n\n" +

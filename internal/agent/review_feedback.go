...
const reviewFeedbackPrefix = "\n\n# Review feedback / 审阅反馈\n\n" +
	...
	"Apply review feedback as a minimal necessary patch, not as permission to regenerate the chapter. First satisfy every applicable comment, then preserve all unaffected prose, strong passages, character voice, plot beats, foreshadowing and continuity. The number of comments does not broaden the authorized edit scope and does not by itself make the task high risk. Group comments by file, make a compact internal patch plan, and combine non-overlapping changes to one file into one replace_lines call. Use the latest numbered body already in your context as the working snapshot (cross-turn assembly keeps it current, including a refreshed body after your own edits); only read a file first when its current numbered body is missing from your context, and then copy its revision to file_revision. Use the returned 1-based line numbers with start_line/end_line. For repeated literal changes such as names or terminology, use replace_text directly; it replaces every current literal match and reports an error without writing when there is no match. If a line range is stale or a revision conflict is reported, rebuild from the current numbered body; only a revision conflict requires re-reading. Never fall back to write_file for an existing chapter unless the user explicitly requested a full-chapter rewrite. If a comment cannot be satisfied without changing a substantially wider range, explain the required scope and ask for confirmation before expanding it.\n\n" +

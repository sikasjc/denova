...
4. 每轮已把 setting/outline.md、setting/character-states.md 和资料库作为有来源、有上限的权威快照注入当前作品状态；默认直接据此判断连贯性，不要为查看它们的内容而重复 read_file。仅当注入内容被截断需要全文，或需要某一章实际正文时，才按路径显式读取；写回 character-states.md 和资料库仍按下方规则进行
...
2. 直接依据已注入的 outline、character-states 和资料库快照确认长期方向、角色状态与真实落点，并结合最近实际章节正文核对；这些快照无需重复 read_file，只有注入被截断需要全文或要看某章正文时才按路径读取
...
### 续写章节时
1. 直接依据已注入的 outline、character-states 快照和常驻资料正文确认长期设定与角色当前状态，不要为查看它们而重复 read_file；只有注入被截断需要全文时才按路径读取。名称目录中出现、但正文未注入的资料，按需用 read_lore_items（已知唯一名称）或 list_lore_items 的筛选和 detail=full 读取
